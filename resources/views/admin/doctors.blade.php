    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            importForm.addEventListener('submit', async function(e) {
                ...
                const cancelBtn = importForm.querySelector('button[type="button"]');
                ...
                if (!allowedTypes.includes(file.type)) {
                    ...
                    resultsContent.innerHTML = `
                        <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded">
                            <h3 class="font-semibold text-red-800 mb-2">✗ Invalid File Type</h3>
                            <p class="text-sm text-red-700">Please upload a valid CSV file.</p>
                        </div>
                    `;
                    return;
                }
                ...
                try {
                    ...
                    progressText.textContent = 'Processing doctors...';
                    ...
                    setTimeout(() => {
                        ...
                        // Render each error individually, split by comma
                        let errorsHtml = '';
                        if (hasErrors && data.data.errors.length > 0) {
                            data.data.errors.forEach((err) => {
                                const splitErrors = err.split(',').map(e => e.trim())
                                    .filter(Boolean);

                                splitErrors.forEach(singleErr => {
                                    const match = singleErr.match(
                                        /^\[([^\]]+)\]\s*(.*)/);
                                    const field = match ? match[1] : 'General';
                                    const message = match ? match[2] :
                                        singleErr;
                                    errorsHtml += `
                                        <div class="text-sm p-2 bg-red-100 rounded border-l-3 border-red-500 text-red-800 mb-1">
                        ...
                        // Show toast only if all rows succeeded
                        if (!hasErrors) {
